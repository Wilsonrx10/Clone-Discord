    ...
    <div class="channel-data">
        ...
        <div v-else>
            ...
            <div v-if="NewFriend.estado" class="newfriend">
                <img :src="'/image/usuarios/'+dadosMensagem.foto_perfil" alt="">
                <strong>{{dadosMensagem.name}}</strong>
            ...
            <div class="input-wrapper">
                <input @keypress.enter="EnviarMensagem" type="text" v-model="mensagem" placeholder="Conversar">
                <div class="icon">
                    <At :size="24" />
                </div>
            </div>
...
watch(()=> bus.value.get('ExibirMessage'),(payload) => {
    wampus.value = false;
    dadosMensagem.value = payload[0];
    store.commit('GUARDAR_DADOS_MENSAGEM_USUARIO',dadosMensagem.value);
    if (payload[1] != null) {
        let resultado = store.state.ListaAmigos.filter(item => item.id === payload[0].id)
...
const EnviarMensagem = () => {
    ...
    axios.post('/channel/EnviarMensagem',{mensagem: mensagem.value,id: dadosMensagem.value.id})
    .then((response) => {
